...
if ( isset( $_POST["ywgc-link-code"] ) ){
    ...
    if ( ! is_object( $gift_card ) || $gift_card->ID == 0 ){
        ...
        if ( is_object( $gift_card ) && $gift_card->ID != 0 ){
            ...
            echo '<div class="yith-add-new-gc-my-account-notice-message" style="font-weight: bolder; color: green;">' . sprintf(esc_html__( "The gift card code %s is now linked to your account.", 'yith-woocommerce-gift-cards' ), $gift_card->get_code()) . '</div>';
        }
    }
}
...
<form method="post" name="form-link-gift-card-to-user" class="form-link-gift-card-to-user" style="width: 65%; float: right; display: none">
						...
						switch ( $column_id ) {
							...
							case 'status' :
								...
								if ( $gift_card->expiration ) {
									$value .= '<br>';
									$value .= apply_filters( 'yith_ywgc_gift_card_custom_expiration_message',
										sprintf( _x( 'Expires on: %s', 'gift card expiration date', 'yith-woocommerce-gift-cards' ), date_i18n( $date_format, $gift_card->expiration ) ), $gift_card );
								}
								break;
