...
if ( ! class_exists ( 'YITH_YWGC_Gift_Cards_Table' ) ) {
	...
	class YITH_YWGC_Gift_Cards_Table {
		...
		function add_custom_columns_title( $defaults ) {
			...
			$columns[ self::COLUMN_ID_ENABLE_DISABLE ]  = esc_html__( "Enable", 'yith-woocommerce-gift-cards' );
			...
			switch ( $column_name ) {
				...
				case self::COLUMN_ID_ACTIONS:
					...
					echo '<p id="ywgc_direct_link" style="display: none">' . $direct_link . '</p>
						  <button type="button" id="ywgc_direct_link_button " class="button ywgc-actions" style="display: none;
						  vertical-align: initial;padding: 4px 10px 6px; box-shadow: rgba(0, 113, 161, 0.3) 0px 0px 5px;border: unset;background-color: white;">' . $copy_text . '</button>
		...
		public function show_download_pdf_button( $post_ID, $gift_card ) {
			if ( $gift_card->is_enabled () && apply_filters('ywgc_show_download_pdf_button', true )) {
				...
				if ( ! empty( $recipient ) ) {
					...
					$download_pdf_link =sprintf ( '<a class="ywgc-actions %s" href="%s" title="%s" style="display: none; box-shadow: rgba(0, 113, 161, 0.3) 0px 0px 5px;"><img src="' . YITH_YWGC_ASSETS_URL . '/images/pdf.svg' .'" alt=""></a>',
		...
		public function show_send_email_button( $post_ID, $gift_card ) {
			if ( $gift_card->is_enabled () ) {
				...
				if ( ! empty( $recipient ) ) {
					...
					$send_now_link = sprintf ( '<a class="ywgc-actions %s" href="%s" title="%s" style="display: none; box-shadow: rgba(0, 113, 161, 0.3) 0px 0px 5px;"><img src="' . YITH_YWGC_ASSETS_URL . '/images/mail.svg' .'" alt=""></a>',
						'gift-cards send-now',
						esc_url_raw ( add_query_arg ( array(
