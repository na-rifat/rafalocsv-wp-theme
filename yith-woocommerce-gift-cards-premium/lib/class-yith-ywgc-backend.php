...
if ( ! class_exists ( 'YITH_YWGC_Backend' ) ) {
	...
	class YITH_YWGC_Backend {
			...
			foreach ( $gift_ids as $gift_id ) {
				...
				if ( ! $gc->is_pre_printed () ):
					...
					<span
						...
						<?php if ( current_user_can('administrator') || current_user_can('manage_woocommerce' ) ){ ?>
		...
		function enqueue_backend_files( $hook ) {
			...
			if ( ( isset( $_REQUEST[ 'page' ] ) && $_REQUEST[ 'page' ] == 'yith_woocommerce_gift_cards_panel' && $_REQUEST[ 'tab' ] != 'export_import' ) || $screen->id == 'edit-giftcard-category') {
			...
			//Register script for the Export and Import features
			if ( isset( $_REQUEST[ 'page' ] ) && $_REQUEST[ 'page' ] == 'yith_woocommerce_gift_cards_panel' && $_REQUEST[ 'tab' ] == 'export_import' ) {

				wp_register_script ( "ywgc_export_import",
					YITH_YWGC_SCRIPT_URL . yit_load_js_file ( 'ywgc-export-import.js' ),
					array(
						'jquery',
						'jquery-blockui',
					),
				...
				wp_localize_script ( 'ywgc_export_import',
					'ywgc_data', array(
						'loader'            => apply_filters ( 'yith_gift_cards_loader', YITH_YWGC_ASSETS_URL . '/images/loading.gif' ),
						'ajax_url'          => admin_url ( 'admin-ajax.php' ),
				...
				wp_enqueue_script ( "ywgc_export_import" );
			...
			if ( ( 'product' == $post_type ) || ( 'gift_card' == $post_type ) || ( 'shop_order' == $post_type ) ||  isset( $_REQUEST[ 'page' ] ) && $_REQUEST[ 'page' ] == 'yith_woocommerce_gift_cards_panel' || "edit-gift_card" == $screen->id ) {
		...
		public function show_sale_discount_settings( $product_id ) {
			...
			<div class="ywgc-form-field yith-plugin-ui" style="margin-left: 160px; margin-top: 1em;">
				...
				<div class="ywgc-form-field__content">
					...
					echo yith_plugin_fw_get_field( array(
						...
						'value' => $discount_settings == "yes" ? 'yes' : 'no',
						'data'  => array(
			...
			<div class="ywgc-add-discount-settings-container ywgc-hidden">
				<p class="form-field sale-discount">
					...
					<input type="text" id="gift_card-sale-discount" name="gift_card-sale-discount" value="<?php echo $sale_discount ?>">
					<span class="ywgc-form-field__description "><?php _e ( "Enter an optional discount for this gift card.", 'yith-woocommerce-gift-cards' ); ?></span>
				</p>
				...
				<p class="form-field sale-discount-text">
					...
					<input type="text" id="gift_card-sale-discount-text" name="gift_card-sale-discount-text" value="<?php echo $sale_discount_text ?>">
		...
		public function show_gift_card_expiration_date_settings( $product_id ) {
			...
			<div class="ywgc-form-field yith-plugin-ui" style="margin-left: 160px; margin-top: 1em;">
				...
				<div class="ywgc-form-field__content">
					...
					echo yith_plugin_fw_get_field( array(
						...
						'value' => $expiration_settings == "yes" ? 'yes' : 'no',
						'data'  => array(
							'product-id' => $product->get_id(),
						)
					) );
			...
			<div class="ywgc-expiration-settings-container ywgc-hidden">
				<p class="form-field expiration-date-field">
					...
					<input type="text" class="ywgc-expiration-date-picker" id="gift-card-expiration-date" name="gift-card-expiration-date" value="<?php echo $expiration_date;?>" data-min-date="<?php echo $expiration_date;?>">
		...
		public function show_gift_card_product_settings() {
			...
			<div class="options_group show_if_gift-card">
				<p class="form-field ywgc-amounts-list">
					<label for="gift_card-amount"><?php _e ( "Gift card amount", 'yith-woocommerce-gift-cards' ); ?></label>
					<?php $this->show_gift_card_amount_list( $thepostid ); ?>
					<span class="wrap add-new-amount-section">
                    <input type="text" id="gift_card-amount" name="gift_card-amount" class="short wc_input_price" style="" placeholder="<?php _e ( "Enter amount", 'yith-woocommerce-gift-cards' ); ?>">
						<span class="ywgc-currency-symbol-enter-amount"><?php echo get_woocommerce_currency_symbol() ?></span>
                    <a href="#" class="add-new-amount"><?php apply_filters('yith_wcgc_add_another_amount_text', _e ( "+ Add another amount", 'yith-woocommerce-gift-cards' ) ); ?></a>
                    </span>
				</p>
			</div>
			...
			<span class="variation-amount-list">
			    ...
			    if ( $amounts ): ?>
				    <?php foreach ( $amounts as $amount ) : ?>
						...
						<input type="text" class="gift_card-amount" name="gift-card-amounts[]" value="<?php _e ( $amount ); ?>">
						<span class="ywgc-currency-symbol-amounts"><?php echo get_woocommerce_currency_symbol() ?></span>
						<a class="remove-amount" href=""><span class="dashicons dashicons-trash"></span></a>
					</span>
		...
		public function create_gift_cards_for_order( $order ) {
			...
			foreach ( $order->get_items ( 'line_item' ) as $order_item_id => $order_item_data ) {
				...
				for ( $i = 0; $i < $quantity; $i ++ ) {
					...
					if ( $gift_card->is_virtual() && "yes" == get_option ( "ywgc_enable_pre_printed_virtual", 'no' ) ||
					     apply_filters ( 'yith_ywgc_custom_condition_set_gift_card_as_preprinted', false, $gift_card ) ) {
						$gift_card->set_as_pre_printed ();
					}
					else if (  !$gift_card->is_virtual() && "yes" == get_option ( "ywgc_enable_pre_printed_physical", 'no' ) ||
					           apply_filters ( 'yith_ywgc_custom_condition_set_gift_card_as_preprinted', false, $gift_card ) ){
						$gift_card->set_as_pre_printed ();
					}
					else {
