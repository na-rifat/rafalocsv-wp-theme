...
if ( ! class_exists( 'YITH_Vendor_Shipping_Frontend' ) ) {
	...
	/**
	 * Class YITH_Vendors_Shipping_Frontend41
	 *
	...
	class YITH_Vendor_Shipping_Frontend {
		...
		private function get_matched_zone( $vendor, $destination_country, $destination_continent, $destination_state, $destination_postcode ) {
			...
			if ( ! empty( $matched_zone_keys ) ) {
				 ...
				 */
				$matched_zone_keys_temp = $matched_zone_keys;
				foreach ( $matched_zone_keys_temp as $zone_position => $zone_key ) {
					if ( ! empty( $vendor->zone_data[ $zone_key ]['zone_post_code'] ) ) {
						...
						foreach ( $postcodes as $postcode ) {
							...
							if ( ! empty( $matched_zone_key ) ) {
								...
								unset( $matched_zone_keys[ $zone_position ] );
							}
						}
					}
		...
		private function get_extra_cost( $vendor, $package, $shipping_method ) {
			...
			foreach ( $shipping_class_costs as $sc_id => $sc_args ) {
				$sc_cost      = $sc_args['cost'];
				$product_qty  = $sc_args['qty'];
				$product_cost = $sc_args['line_total'];

				$sc_cost = do_shortcode(
					str_replace(
						array(
							'[qty]',
							'[cost]',
				...
				// Remove whitespace from string.
				$sc_cost = preg_replace( '/\s+/', '', $sc_cost );

				// Remove locale from string.
				$sc_cost = str_replace( $decimals, '.', $sc_cost );

				// Trim invalid start/end characters.
				$sc_cost = rtrim( ltrim( $sc_cost, "\t\n\r\0\x0B+*/" ), "\t\n\r\0\x0B+-*/" );

				// Do the math.
				$sc_cost = $sc_cost ? WC_Eval_Math::evaluate( $sc_cost ) : 0;
