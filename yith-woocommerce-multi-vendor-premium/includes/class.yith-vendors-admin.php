...
if ( ! class_exists( 'YITH_Vendors_Admin' ) ) {
	...
	class YITH_Vendors_Admin {
		...
		public function __construct() {
			...
			add_action( 'admin_menu', array( $this, 'add_shop_orders_menu_item' ), 5 );
			...
			add_action( 'admin_notices', array( $this, 'remove_update_nag' ), 2 );
			...
			add_action( 'admin_menu', array( $this, 'remove_pages_for_vendors' ) );
		 ...
		 * @author Andrea Grillo <andrea.grillo@yithemes.com>
		 */
		public function remove_pages_for_vendors() {
			/* Remove Media Library */
			$vendor = yith_get_vendor( 'current', 'user' );
			if ( $vendor->is_valid() && $vendor->has_limited_access() ) {
				$to_remove = apply_filters( 'yith_wpv_vendor_to_remove_menu_items', array( 'edit.php', 'tools.php', 'upload.php', 'woocommerce' ) );
		...
		public static function json_search_admins() {
			...
			if ( ! $vendor_is_valid || ( $vendor_is_valid && 'yes' == get_option( 'yith_wpv_vendors_option_order_management', 'no' ) ) ) {
				return;
			}
		 ...
		 *
		 * @param $suppress bool the current value. Tru by default
