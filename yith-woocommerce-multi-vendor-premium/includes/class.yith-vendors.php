...
if ( ! class_exists( 'YITH_Vendors' ) ) {
	...
	class YITH_Vendors {
		...
		public function __construct() {
			 ...
			 */
			$theme = wp_get_theme();
			if( $theme instanceof WP_Theme && 'hello-elementor' == $theme->get_template() ){
				$this->require['frontend'][] = 'includes/modules/module.yith-hello-elementor-theme.php';
			}
		...
		{
			$option = get_option('yith_wpv_vendors_registration_required_privacy_policy', 'no' );
		...
		public function get_last_modified_data_privacy_policy(){
			...
			$data_last_modified = get_post_datetime( $privacy_page_id,'modified');
			$data_last_modified = $data_last_modified instanceof DateTimeImmutable ? $data_last_modified->format('Y-m-d H:i:s') :'';
		...
		public function get_last_modified_data_terms_and_conditions(){
			...
			$data_last_modified = get_post_datetime( $terms_page_id,'modified');

			if( $data_last_modified !== false ){
				$data_last_modified = $data_last_modified->format('Y-m-d H:i:s');
			}
