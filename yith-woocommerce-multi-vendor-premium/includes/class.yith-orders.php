...
if ( ! class_exists( 'YITH_Orders' ) ) {
	...
	class YITH_Orders {
		...
		public function __construct() {
			...
			add_action( 'before_delete_post', array( $this, 'delete_order_items' ),20 );
		...
		public function create_suborder( $parent_order, $vendor_id, $vendor_products, $posted ) {
			...
			if ( ! empty( $suborder_id ) && ! is_wp_error( $suborder_id ) ) {
				 ...
				 * Custom Meta Data
				 */
				$custom_order_meta = $parent_order->get_meta_data();
				foreach ( $custom_order_meta as $meta_key => $meta_value ) {
				...
				$suborder->set_order_key( apply_filters( 'woocommerce_generate_order_key', uniqid( 'order_' ) ) );
		...
		public static function get_suborder( $parent_order_id = false ) {
			...
			if ( $parent_order_id ) {
				...
				if( defined('YWSON_PREMIUM')){
					...
					$query = "SELECT DISTINCT ID FROM {$wpdb->posts} AS p LEFT JOIN {$wpdb->postmeta} as pm
							  ON p.ID = pm.post_id WHERE
							  p.ID = %s AND p.post_parent = 0 OR( pm.meta_key IN('_ywson_custom_number_order_complete','ywson_custom_quote_number_order') AND pm.meta_value LIKE %s )
						";

					$query = $wpdb->prepare($query, $parent_order_id, '%'.$wpdb->esc_like( $parent_order_id ).'%');
		...
		public function shop_order_columns( $order_columns ) {
			...
			if ( $vendor->is_super_user() ) {
				...
				$hide_customer_section = get_option( 'yith_wpv_vendors_option_order_hide_shipping_billing', 'no' );
				if( 'yes' === $hide_customer_section ){
