	...
	class YITH_Multi_Vendor_Shortcodes {
		...
		public static function load() {
			...
			$shortcodes = array(
				...
				'yith_wcmv_vendor_store_header' => 'YITH_Multi_Vendor_Shortcodes::vendor_store_header'
			);
		...
		public static function vendors_list( $sc_args = array() ) {
			...
			$vendors_args = array(
				...
				'hide_empty' 		=> $sc_args['hide_no_products_vendor'],
			);
		...
		public static function vendor_products( $sc_args = array() ) {
			...
			if ( ! empty( $sitepress ) ) {
				$term_slug         = yit_wpml_object_id( $default_vendor_id, YITH_Vendors()->get_taxonomy_name(), true, wpml_get_default_language() );
				$default_term      = get_term( $term_slug, YITH_Vendors()->get_taxonomy_name() );
				$default_vendor_id = $default_term instanceof WP_Term ? $default_term->term_id : $default_vendor_id;
			}
		 ...
		 * @author Andrea Grillo <andrea.grillo@yithemes.com>
		 */
		public function vendor_store_header( $sc_args = array() ) {
