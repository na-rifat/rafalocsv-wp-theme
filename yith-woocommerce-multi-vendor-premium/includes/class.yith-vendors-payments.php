	...
	class YITH_Vendors_Payments {
		...
		public function get_payments_details_by_commission_ids ( $commission_id, $select = array(), $where = array() ){
			...
			$query = "SELECT DISTINCT $select_query, relationship.commission_id
					  FROM {$wpdb->prefix}yith_vendors_payments_relationship AS relationship JOIN
					  	   {$wpdb->prefix}yith_vendors_payments AS payments ON relationship.payment_id = payments.ID 
			...
			$result = $wpdb->get_results( $query.$where_clause, ARRAY_A );
