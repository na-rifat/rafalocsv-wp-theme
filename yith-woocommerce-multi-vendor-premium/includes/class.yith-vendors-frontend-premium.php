...
if ( ! class_exists( 'YITH_Vendors_Frontend_Premium' ) ) {
	...
	class YITH_Vendors_Frontend_Premium extends YITH_Vendors_Frontend {
		...
		public function __construct() {
			...
			if ( YITH_Vendors()->is_wc_2_7_or_greather ) {
				add_action( 'woocommerce_archive_description', array(
					$this,
					'remove_woocommerce_term_description'
				), 5 );
				...
				if ( YITH_Vendors()->is_wc_3_3_or_greather ) {
					switch ( $related ) {
						...
						case 'vendor':
							add_filter( 'woocommerce_related_products', array(
								$this,
								'get_vendor_related_product'
							), 10, 3 );
							break;
					}
				} else {
			...
			if ( 'yes' == $revision_management && ( 'yes' == $privacy_required || 'yes' == $terms_required ) ) {
				...
				//Accept terms and privacy management
				add_action( 'wp_loaded', array( $this, 'check_vendor_terms_of_service' ), 20 );
			}
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'product-vendors', YITH_WPV_ASSETS_URL . 'js/' . $js_file, array(
				'jquery',
				'imagesloaded'
			), YITH_WPV_VERSION, true );
			...
			wp_register_script( 'gmap3', YITH_WPV_ASSETS_URL . 'third-party/gmap3/gmap3.min.js', array(
				'jquery',
				'gmaps-api'
			), '6.0.0', false );
			...
			if ( is_singular( 'product' ) ) {
				...
				wp_register_script( 'yith-wpv-prettyPhoto-init', YITH_WPV_ASSETS_URL . 'js/init.prettyPhoto' . $suffix . '.js', array(
					'jquery',
					'prettyPhoto'
				), YITH_WPV_VERSION, true );
				if ( $show_report_abuse && ( $lightbox_disabled || YITH_Vendors()->is_wc_2_7_or_greather ) ) {
					...
					if( function_exists( 'flatsome_woocommerce_scripts_styles' ) ){
						remove_action( 'wp_enqueue_scripts', 'flatsome_woocommerce_scripts_styles', 98 );
						add_action( 'wp_enqueue_scripts', 'yith_wcmv_flatsome_support', 98 );
					}
				}
		...
		public function process_registration( $validation_error, $check_type = 'new_vendor' ) {
			...
			if ( isset( $_POST['vendor-register'] ) ) {
				...
				} elseif ( empty( $_POST['vendor-privacy'] ) && YITH_Vendors()->is_privacy_policy_require() ) {
					$validation_error->add( 'privacy_policy', $this->get_registration_error_string( 'privacy_policy' ) );
					'switch_customer' == $check_type && wc_add_notice( '<strong>' . __( 'Error', 'yith-woocommerce-product-vendors' ) . ':</strong> ' . $this->get_registration_error_string( 'privacy_policy' ), 'error' );
				} else {
			...
			return apply_filters( 'yith_wcmv_process_registration', $validation_error, $check_type );
		}
		...
		public function create_vendor( $customer_id, $force_init_wc_mail = false ) {
			...
			if ( isset( $_POST['vendor-terms'] ) ) {

				$data_last_modified               = YITH_Vendors()->get_last_modified_data_privacy_policy();
				$vendor->data_terms_and_condition = $data_last_modified;
			}
			if ( isset( $_POST['vendor-privacy'] ) ) {
				$data_last_modified          = YITH_Vendors()->get_last_modified_data_terms_and_conditions();
				$vendor->data_privacy_policy = $data_last_modified;
			}
		...
		public function send_report_abuse() {
			...
			if ( $check ) {
				...
				$message = sprintf( __( "User %s (%s) is reporting an abuse on the following product:", 'yith-woocommerce-product-vendors' ), $name, $from_email );
				$message .='\n';
				$message .= sprintf( __( "Product details: %s (ID: #%s)", 'yith-woocommerce-product-vendors' ), $product->post_title, $product->ID );
				$message .='\n';

				if ( $vendor->is_valid() ) {
					$message .= sprintf( __( "Vendor shop: %s (ID: #%s)", 'yith-woocommerce-product-vendors' ), $vendor->name, $vendor->id );
					$message .='\n';
				}

				$message .= sprintf( __( "Message: %s", 'yith-woocommerce-product-vendors' ), $user_message );
				$message .= "\n\n\n";

				$message .= sprintf( __( "Product page: %s", 'yith-woocommerce-product-vendors' ), get_the_permalink( $product->ID ) );
				$message .='\n';

				if ( $vendor->is_valid() ) {
					$message .= sprintf( __( "Vendor Account details: %s", 'yith-woocommerce-product-vendors' ), $vendor->get_url( 'admin' ) );
					$message .='\n';
				}
		...
		public function check_vendor_terms_of_service() {
			...
			if ( isset( $_REQUEST['yith_mv_accept_temrs_and_privacy_nonce'] ) && wp_verify_nonce( $_REQUEST['yith_mv_accept_temrs_and_privacy_nonce'], 'yith-mv-accept-terms-and-privacy' ) ) {

				$vendor_id        = isset( $_REQUEST['yith_vendor_id'] ) ? $_REQUEST['yith_vendor_id'] : '';
				$privacy_accepted = isset( $_REQUEST['yith_mv_accept_privacy'] ) ? 'yes' : 'no';
				$terms_accepted   = isset( $_REQUEST['yith_mv_accept_terms'] ) ? 'yes' : 'no';

				$vendor = yith_get_vendor( $vendor_id, 'vendor' );
				$restore_vendor = true;

				if( YITH_Vendors()->is_terms_and_conditions_require() ) {
					if ( 'yes' == $terms_accepted ) {
						$vendor->data_terms_and_condition = YITH_Vendors()->get_last_modified_data_terms_and_conditions();
				...
				if( YITH_Vendors()->is_privacy_policy_require() ) {
					if ( 'yes' == $privacy_accepted ) {
						$vendor->data_privacy_policy = YITH_Vendors()->get_last_modified_data_privacy_policy();
					}else{
						$restore_vendor = false;
					}
				}

				if( $restore_vendor && !yith_plugin_fw_is_true( $vendor->enable_selling ) ){
					$vendor->enable_selling = 'yes';
				}

				$my_account_url = wc_get_page_permalink('myaccount');
				wp_safe_redirect(wc_get_endpoint_url('terms-of-service','',$my_account_url));
				exit;
			}
		}
	}
