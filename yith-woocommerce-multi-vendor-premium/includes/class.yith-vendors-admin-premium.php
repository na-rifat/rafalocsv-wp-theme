...
if ( ! class_exists( 'YITH_Vendors_Admin_Premium' ) ) {
	...
	class YITH_Vendors_Admin_Premium extends YITH_Vendors_Admin {
		...
		public function __construct() {
			...
			add_filter( "manage_toplevel_page_{$commissions_screen}_columns", array(
				$this,
				'add_commissions_screen_options'
			) );
			...
			add_action( 'admin_menu', array( $this, 'report_menu' ), 99 );
			...
			add_action( 'wp_ajax_wpv_vendors_force_skip_review_option', array(
				YITH_Vendors(),
				'force_skip_review_option'
			) );
			...
			add_action( $this->_taxonomy_name . '_edit_form_fields', array(
				$this,
				'add_wp_editor_to_vendor_tax'
			), 5, 2 );
			...
			add_filter( 'woocommerce_duplicate_product_capability', array(
				$this,
				'enabled_duplicate_product_capability'
			) );
			...
			add_action( 'yith_wcmv_save_post_product', array(
				$this,
				'set_product_to_pending_review_after_edit'
			), 10, 3 );

			add_action( 'wp_ajax_woocommerce_save_variations', array(
				$this,
				'set_product_to_pending_review_after_ajax_save'
			), 5 );
			...
			//Manage update option , for terms & condition and privacy policy pages
			$manage_revision = get_option( 'yith_wpv_manage_terms_and_privacy_revision', 'no' );
			if ( 'yes' == $manage_revision ) {
				add_action( 'update_option_yith_wpv_manage_terms_and_privacy_revision_actions', array(
					$this,
					'apply_revision_actions'
				), 10, 3 );
				add_action( 'update_option_yith_wpv_manage_terms_and_privacy_revision_disable_after', array(
					$this,
					'reschedule_disable_event'
				), 10, 3 );
				add_action( 'admin_notices', array( $this, 'print_check_revision_message' ), 20 );
		...
		public function vendor_settings() {
			...
			if ( ! empty( $menu ) ) {
				...
				if ( $vendor->is_super_user() ) {
					$pending_vendors = count( YITH_Vendors()->get_vendors( array(
						'pending' => 'yes',
						'fields'  => 'ids'
					) ) );
				} elseif ( $vendor->is_valid() ) {
					$pending_commissions = count( YITH_Commissions()->get_commissions( array(
						'vendor_id' => $vendor->id,
						'status'    => 'pending'
					) ) );
				...
				if ( $pending_vendors > 0 || $pending_commissions > 0 || $processing_orders > 0 ) {
					...
					if ( $pending_vendors > 0 ) {
						...
						$vendor_taxonomy_uri = htmlspecialchars( add_query_arg( array(
		...
		public function get_columns( $columns ) {
			...
			$to_add = array(
				...
				'privacy_policy_date'   => __( 'Privacy Policy', 'yith-woocommerce-product-vendors' ),
				'terms_conditions_date' => __( 'Terms and conditions', 'yith-woocommerce-product-vendors' ),
			...
			if ( 'yes' === get_option( 'yith_wpv_manage_terms_and_privacy_revision', 'no' ) ) {
				if ( empty( get_option( 'yith_wpv_terms_and_conditions_page_id' ) ) ) {
					unset( $to_add['terms_conditions_date'] );
				}

				if ( empty( get_option( 'yith_wpv_privacy_page' ) ) ) {
					unset( $to_add['privacy_policy_date'] );
				}
			}
		...
		public function custom_columns( $custom_column, $column_name, $term_id ) {
			...
			switch ( $column_name ) {
				...
				case 'privacy_policy_date':
					return YITH_Vendor::get_date( $vendor->data_privacy_policy, 'display' );;
					break;

				case 'terms_conditions_date':
					return YITH_Vendor::get_date( $vendor->data_terms_and_condition, 'display' );;
					break;
			}
		}
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'yith-wpv-datepicker', YITH_WPV_ASSETS_URL . 'js/init.datepicker.js', array(
				'jquery',
				'jquery-ui-datepicker'
			), YITH_Vendors()->version, true );
			$localize = array(
			...
			if ( 'no' == $vendor->featured_products && 'post-new.php' == $pagenow && ! empty( $_GET['post_type'] ) && 'product' == $_GET['post_type'] ) {
		...
		public function process_bulk_action() {
			...
			if ( in_array( $action, array_keys( $commission_status ) ) ) {
				...
				if ( $send_commissions_email_on_bulk_action ) {
					remove_filter( 'yith_wcmv_send_commission_paid_email', '__return_true' );
					add_filter( 'yith_wcmv_send_commission_paid_email', '__return_false' );
					WC()->mailer();
				}
				...
				if ( $send_commissions_email_on_bulk_action ) {
					...
					remove_filter( 'yith_wcmv_send_commission_paid_email', '__return_false' );
				}
		...
		public function admin_field_button( $value ) {
			...
			<tr valign="top">
				...
				<td class="forminp">
					<input type="button" name="force_review" id="<?php echo $value['id'] ?>"
						   value="<?php echo $value['name'] ?>" class="button-secondary"/>
		...
		public function single_product_commission_tab( $product_data_tabs ) {
			if ( apply_filters( 'yith_wcmv_show_single_product_commission_tab_capability', current_user_can( 'manage_woocommerce' ) ) ) {
		...
		public function save_product_commission_meta( $post_id, $post ) {
			...
			if( apply_filters( 'yith_wcmv_show_single_product_commission_tab_capability', $vendor->is_super_user() ) ){
				// Save Product Commission Rate
				$product = wc_get_product( $post_id );
				if ( $product instanceof WC_Product ) {
					if ( ! empty( $_POST['yith_wpv_product_commission'] ) ) {
						$product->add_meta_data( '_product_commission', esc_attr( $_POST['yith_wpv_product_commission'] ), true );
					} else {
		...
		public function allowed_wc_post_types() {
			...
			if ( $vendor->is_valid() && $vendor->has_limited_access() ) {
				...
				if ( ! $enabled ) {
					wp_die( $this->get_product_amount_limit_message( $vendor ) );
				}
			}
		}
							...
							<p>
								...
								if ( ! $is_vendor_taxonomy_page ) {
									printf( ' <a href="%s">%s</a>', esc_url( add_query_arg( array(
										...
										'taxonomy'  => YITH_Vendors()->get_taxonomy_name(),
										'orderby'   => 'owner'
									), admin_url( 'edit-tags.php' ) ) ), __( 'Go to Vendor page to fix it.', 'yith-woocommerce-product-vendors' ) );
								}
								?>
							</p>
						<?php endif; ?>
						...
						<p>
							...
							if ( ! $is_vendor_details_page ) {
								printf( ' <a href="%s">%s</a>', esc_url( add_query_arg( array(
									...
									'tab'  => 'vendor-settings'
								), admin_url( 'admin.php' ) ) ), __( 'Go to Vendor details page to fix it.', 'yith-woocommerce-product-vendors' ) );
							}
							?>
						</p>
					</div>
		...
		public static function create_plugins_page() {
			...
			$privacy_page = get_option( 'yith_wpv_privacy_page', false );

			//Create a privacy policy page
			if ( $privacy_page === false ) {
				wc_create_page( 'Vendors Privacy Policy', 'yith_wpv_privacy_page', __( 'Privacy Policy for Vendors', 'yith-woocommerce-product-vendors' ), '', 0 );
			}
		}
			...
			if ( $tag instanceof WP_Term && 'yes' == get_option( 'yith_wpv_vendors_option_editor_management', 'no' ) && $this->is_vendor_tax_page() ) {
				...
				<tr class="form-field">
					<th scope="row" valign="top"><label
							for="description"><?php _ex( 'Description', 'Taxonomy Description', 'yith-woocommerce-product-vendors' ); ?></label>
					</th>
					<td>
						<?php $this->add_wp_editor( $vendor->description, array(
							'textarea_name' => 'description',
							'textarea_id'   => 'textarea#description'
						), true ); ?>
						<br/>
						<span
							class="description"><?php _e( 'The description is not prominent by default; however, some themes may show it.', 'yith-woocommerce-product-vendors' ); ?></span>
					</td>
				</tr>
				<?php
			}
		 ...
		 * @author Andrea Grillo <andrea.grillo@yithemes.com>
		 *
		 * @return string the vendor capability name
		 */
		public function enabled_duplicate_product_capability( $cap ) {
			$vendor = yith_get_vendor( 'current', 'user' );
			if ( $vendor->is_valid() && $vendor->has_limited_access() ) {
				if ( 'yes' === get_option( 'yith_wpv_enable_product_amount', 'no' ) ) {
					$products_limit = apply_filters( 'yith_wpv_vendors_product_limit', get_option( 'yith_wpv_vendors_product_limit', 25 ), $vendor );
					$products_count = count( $vendor->get_products( array( 'post_status' => 'any' ) ) );
					if ( $products_limit > $products_count ) {
	  ...
	  /*
	   * Check if current vendor untrashed a produc
	   ...
	   */
		public function pre_untrash_product( $check, $post ) {
			if ( 'product' === $post->post_type ) {
				...
				if ( $vendor->is_valid() ) {
					...
					$products_count = count( $vendor->get_products( array( 'post_status' => 'any' ) ) );
					if ( $products_limit <= $products_count ) {
						printf( __( '<div class="wp-die-message">You are not allowed to create more than %1$s products. %2$sClick here to return in your admin area%3$s.</div>', 'yith-woocommerce-product-vendors' ), $products_limit, '<a href="' . esc_url( 'edit.php?post_type=product' ) . '">', '</a>' );
						$check = false;
					}
		...
		*/
		...
		public function action_links( $links ) {
			$links = yith_add_action_links( $links, $this->_panel_page, true, YITH_WPV_SLUG );
		...
		public function apply_revision_actions( $old_value, $value, $option ) {
			...
			if ( 'yith_wpv_manage_terms_and_privacy_revision_actions' == $option && $old_value !== $value ) {

				if ( 'no_action' == $value || 'disable_now' == $value ) {

					$timestamp = wp_next_scheduled( 'yith_mv_disable_vendor_to_sale_cron' );
					if ( $timestamp ) {
						wp_clear_scheduled_hook( 'yith_mv_disable_vendor_to_sale_cron' );
					}
				}

				if ( 'disable_now' == $value ) {

					do_action( 'yith_mv_disable_vendor_to_sale' );

				}

				if ( 'disable_after' == $value ) {

					$days      = get_option( 'yith_wpv_manage_terms_and_privacy_revision_disable_after', 3 );
					$timestamp = time() + ( $days * DAY_IN_SECONDS );
					wp_schedule_single_event( $timestamp, 'yith_mv_disable_vendor_to_sale_cron' );
				}
			}
		 ...
		 * @param string $value
		 * @param string $option
		 * delete the old cron event, and create the new
		 */
		public function reschedule_disable_event( $old_value, $value, $option ) {
			...
			$timestamp = time() + ( $value * DAY_IN_SECONDS );
			wp_schedule_single_event( $timestamp, 'yith_mv_disable_vendor_to_sale_cron' );
		 ...
		 */
		 ...
		 public function get_revision_message(){
			...
			if ( $vendor->is_valid() ) {
				...
				$endpoint_url  = wc_get_endpoint_url( 'terms-of-service', '', $myaccount_url );

				$is_terms              = $terms_req && ! $terms_accepted;
				$is_privacy            = $privacy_req && ! $privacy_accepted;
				$message               = '';
				$last_terms_modified   = YITH_Vendors()->get_last_modified_data_terms_and_conditions();
				...
				if ( $is_terms && $is_privacy ) {
					...
					if ( $page_title ) {
						if ( 'disable_now' == $action ) {
							...
							$max_last_modified = $is_terms ? $last_terms_modified : $last_privacy_modified;
							$today             = current_time( 'Y-m-d' );
							$max_last_modified += ( $days * DAY_IN_SECONDS );
