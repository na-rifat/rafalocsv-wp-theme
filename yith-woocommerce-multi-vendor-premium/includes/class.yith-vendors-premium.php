...
if ( ! class_exists( 'YITH_Vendors_Premium' ) ) {
	...
	class YITH_Vendors_Premium extends YITH_Vendors {
		...
		public function __construct() {
			...
			add_action( 'yith_mv_disable_vendor_to_sale', array( $this, 'disable_vendors_to_sale' ) );
			add_action( 'yith_mv_disable_vendor_to_sale_cron', array( $this, 'disable_vendors_to_sale' ) );
			parent::__construct();
		...
		public function get_commission( $commission, $vendor_id, $vendor, $product_id ) {
			...
			if( isset( $vendor->commission ) ){
				$commission = ! empty( $vendor->commission ) ? $vendor->commission / 100 : 0;
			}
		...
		public function update_acceptance_details_for_vendors( $post_id ) {
			...
			if ( ( ( $terms_req && $post_id == $terms_page_id ) || ( $privacy_req && $post_id == $privacy_page_id ) ) && 'publish' == get_post_status( $post_id ) ) {
				...
				if ( 'disable_now' == $action ) {
					...
					if ( ! wp_next_scheduled( 'yith_mv_disable_vendor_to_sale_cron' ) ) {
		...
		public function disable_vendors_to_sale() {
			 ...
			 */
			foreach ( $vendors as $vendor ) {
				$terms_check = YITH_Vendors()->is_terms_and_conditions_require();
				$terms_check = ! $terms_check || ( $terms_check && $vendor->has_terms_and_conditions_accepted() );

				$privacy_check = YITH_Vendors()->is_privacy_policy_require();
