...
<div id="yith-become-a-vendor" class="woocommerce shortcodes">
    ...
    <form method="post" class="register">
        ...
        <?php endif; ?>
		<?php
		if( $is_privacy_policy_require ) :?>
			<p class="form-row form-row-wide">
				<input type="checkbox" class="input-checkbox" name="vendor-privacy" <?php checked( apply_filters( 'yith_wcmv_terms_is_checked_default', isset( $_POST['vendor-terms'] ) ), true ); ?> id="vendor-terms"  />
				<label for="vendor-privacy" class="checkbox"><?php printf( __( 'I&rsquo;ve read and accept the <a href="%s" target="_blank">vendor privacy policy</a>', 'yith-woocommerce-product-vendors' ), esc_url( get_permalink( get_option( 'yith_wpv_privacy_page' ) ) ) ); ?> <span class="required">*</span></label>
