...
function candmReadmore() {
    let $ = jQuery;

    $(`.candm-read-more .read-more-content`).slideUp();

    $(`.candm-read-more .read-more-button`).on(`click`, function (e) {
        e.preventDefault();
        ...
        if (parent.data(`collapsed`) == true) {
            ...
            content.slideDown();
            button.html(`<i class="fas fa-plus"></i>&nbsp;Read less`);
        } else {
            $(parent).data(`collapsed`, true);
            content.slideUp();
            button.html(`<i class="fas fa-plus"></i>&nbsp;Read more`);
        }
    });
}
