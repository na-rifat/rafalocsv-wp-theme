        ...
        public function load_scripts( $end_point ) {
            ...
            switch ( $end_point ) {
                ...
                case 'wcfm-applications':
                    ...
                    $wcfm_screen_manager      = get_option( 'wcfm_screen_manager', array() );
                    $wcfm_screen_manager_data = array();
                    if ( isset( $wcfm_screen_manager['applications'] ) ) {
                        $wcfm_screen_manager_data = $wcfm_screen_manager['applications'];
                    }
			...
			FROM {$wpdb->posts} as posts";
                ...
                function generateTaxonomyHTML( $taxonomy, $product_taxonomies, $selected_taxonomies, $nbsp = '', $is_checklist = false, $is_custom = false, $is_hierarchical = true, $is_children = false, $super_parent = 0 ) {
                    ...
                    foreach ( $product_taxonomies as $cat ) {
                        ...
                        if ( apply_filters( 'wcfm_is_allow_hide_uncatorized', false, $cat->term_id ) && (  ( $cat->term_id == $default_category_id ) || ( in_array( $cat->slug, array( 'uncategorized', 'uncategorised' ) ) ) ) ) {
                            continue;
                        }

                        $term_meta_vendor = get_term_meta( $cat->term_id, '_wcfm_vendor', true );
                        if ( $term_meta_vendor && wcfm_is_vendor() && apply_filters( 'wcfm_is_allow_restrict_vendor_own_term', false ) && ( $term_meta_vendor != $WCFMmp->vendor_id ) ) {
                            continue;
                        }
                        ...
                        if ( ! $is_children ) {
                            ...
                            //$checklis_label_class = 'product_cats_checklist_item_hide_by_cap';
                        }
